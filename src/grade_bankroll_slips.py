...
def find_latest_slip_date():
    files = sorted(SLIPS_DIR.glob("builder_slips_*.csv"))
    if not files:
        raise FileNotFoundError("❌ No bankroll slips found in daily_slips directory.")
    latest = files[-1].stem.replace("builder_slips_", "")
    return latest
...
def main():
    ...
    if args.date is None:
        target_date = find_latest_slip_date()
        print(f"📅 No date provided — using latest slip file: {target_date}")
    else:
    ...
    for _, r in slips.iterrows():
        ...
        if missing > 0:
            ...
            slip_outcome = f"{wins}/3"
            mult = payout_mult(wins)
    ...
    # Build daily stats (no legs column needed)
    day_stats = (
        results[results["slip_outcome"] != "MISSING"]
        .groupby(["slip_outcome"], as_index=False)
        .agg(
            slips=("slip_id", "count"),
    ...
    print("\n📊 Daily slip outcomes:")
    print(results["slip_outcome"].value_counts().to_string())
